...
- 1차: 공통 표지 · 목차 · 이벤트 · 응모권 · 관리자 → 트랙별 아키텍처 · 질문 → 공통 감사합니다, 프론트 10장 / 백엔드 15장(개요·진행 상태·처리 흐름·검증 계획 포함)
...
표지 → 목차 → 프로젝트 개요 → 진행 상태 → 요구사항(이벤트·응모권·관리자) → 기술 스택 → 시스템 아키텍처 → ERD → 응모 처리 흐름 → 추첨·결과 발표 흐름 → 검증 계획 → 질문 → 감사합니다.
