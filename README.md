...
- 프론트 목차: **요구사항 → 게임 컨셉 → 기술 스택 → 아키텍처 → 질문**. 백엔드는 아키텍처 다음 **ERD** 항목 추가. 요구사항 세부 페이지: 이벤트 · 응모권 · 관리자. 마지막은 감사합니다 페이지.
...
표지 → 목차 → 프로젝트 개요 → 진행 상태 → 요구사항(이벤트·응모권·관리자) → 기술 스택 → 시스템 아키텍처 → ERD → 응모 처리 흐름 → 추첨·결과 발표 흐름 → 질문 → 감사합니다.
...
요구사항 다음에 게임 컨셉 페이지를 추가했습니다. 타코야끼 만들기는 요구사항의 후보이며 확정된 게임이 아닙니다. 플레이 흐름은 초안이고, 조작·규칙·화면·연동 상세는 추가 예정입니다. 점수 판정과 보상은 요구사항 기준으로 표시했습니다. `makeSlides()`의 `title: '게임 컨셉'` 객체에서 내용을 갱신합니다.

## 기술 스택

두 트랙 모두 기술 스택 페이지가 있으며 각 목차에서 이동 가능합니다. 백엔드는 로컬 Gradle 설정에 있는 언어·서버·데이터 접근·빌드·테스트 항목을 기록했습니다. MySQL·Redis는 팀 선택을 반영했고, 버전·Redis 적용 범위·배포 상세는 추가 예정입니다. 프론트는 GETDDO/getddo-fe main의 README 외 설정 파일이 없어 기술을 추정하지 않고 항목별 추가 예정으로 표시했습니다. 저장소 설정과 팀 선택이 바뀌면 해당 `title: '기술 스택'` 객체도 갱신하세요.
...
백엔드 기술 스택에서 JUnit 표시를 제거하고 MySQL·Redis 아이콘을 추가했습니다. 이 변경은 발표 자료에만 적용하며 백엔드 의존성 설정은 변경하지 않습니다.

프론트 디자인 시스템·마스코트 소개 추가. 질문은 웹→앱 마이그레이션으로 교체. 제공된 `assets/mascot.svg`를 공통 표지·마무리에 사용. 보조 색상은 방향을 보여주는 예시이며 마젠타는 #E6007E.

양 트랙 공통 프로젝트 개요 5장: 목표·타겟, 문제 정의, 해결 방향, 유사 서비스 분석, 차별점. 유사 서비스는 팀 제공 분석 요약이며 현행 기능·공개 범위는 출처 확인 필요.

표지의 기존 응모권 아트 복원. 캐릭터는 응모권 주변과 마무리 화면의 작은 장식으로 배치. 발표 슬라이드의 사용자 규모·RPS·응답 시간 목표 삭제.

통합 발표자료: 메인에서 선택, `#/mentoring/1/combined/1` 경로. 공통 개요·요구사항은 한 번만 표시하고 프론트·백엔드 내용과 질문을 연결한 25장 구성. 기존 개별 자료에서 자동으로 가져옴.

프론트 확정 기술 20개를 기본·상태 / UI·입력 / 모션 / 테스트·개발 도구 4장에 표시. 프론트 20장, 통합 28장. 버전과 상태 관리·모션 도구의 역할 분담은 추가 예정.

ERD·아키텍처 슬라이드와 목차 제거. Gradle 역할은 빌드 도구, 설계 자료 상태는 진행 중. 현재 프론트 19장 / 백엔드 17장 / 통합 25장.
