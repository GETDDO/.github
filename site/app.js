...
function renderBody(slide) {
  switch (slide.type) {
    ...
    case 'cover': return `<div class="cover-copy"><h1>${lines(slide.title)}</h1><p>${lines(slide.description)}</p><div class="slide-tags">${(slide.tags || []).map(tag => `<span>${escape(tag)}</span>`).join('')}</div></div><img class="cover-mascot" src="assets/mascot.svg" alt="GETDDO 타코야끼 캐릭터">`;
