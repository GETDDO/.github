...
function renderBody(slide) {
  switch (slide.type) {
    case 'tech': return `<div class="tech-layout ${slide.pending ? 'is-pending' : ''}"><div class="tech-list">${slide.items.map(item => `<article class="tech-entry"><img src="assets/tech/${escape(item.icon)}.svg" alt="" width="58" height="58"><div><div class="tech-name"><h2>${escape(item.name)}</h2>${item.version ? `<span class="tech-version">${escape(item.version)}</span>` : ''}</div><p>${escape(item.role)}</p>${item.status ? `<span class="tech-status">${escape(item.status)}</span>` : ''}</div></article>`).join('')}</div><aside class="tech-notes" aria-label="${escape(slide.notesTitle)}"><span class="eyebrow">${escape(slide.notesTitle)}</span>${slide.notes.map(note => `<div><h3>${escape(note.title)}</h3><p>${escape(note.text)}</p></div>`).join('')}</aside></div>`;
