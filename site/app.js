...
function ticketArt() {
  return `<div class="ticket-art" aria-hidden="true">
    ...
    <img class="ticket-mascot" src="assets/mascot.svg" alt="">
  ...
  app.innerHTML = `<div class="home-shell">
    ...
    <main id="main" tabindex="-1">
      ...
      <div class="round-grid">${rounds.map(round => {
        ...
        return `<article class="round-card ${ready ? 'ready' : ''}"><div class="round-top"><span class="round-number">${number(round.id)}</span><span class="status ${ready ? 'open' : ''}">${ready ? '<i></i> 발표 자료' : '준비 중'}</span></div><span class="round-stage">${escape(round.stage)}</span><h3>${escape(round.title)}</h3><h4>${escape(round.subtitle)}</h4><p>${escape(round.description)}</p><div class="track-links">${Object.entries(tracks).map(([key, track]) => available(round.decks[key]) ? `<a href="${route(round.id, key)}"><span class="track-icon">${key === 'combined' ? '＋' : key === 'frontend' ? '〈/〉' : '{ }'}</span><span>${track.name}<small>${track.english}</small></span><span class="link-arrow">↗</span></a>` : `<button disabled aria-label="${round.title} ${track.name} 준비 중"><span class="track-icon">${key === 'combined' ? '＋' : key === 'frontend' ? '〈/〉' : '{ }'}</span><span>${track.name}<small>${track.english}</small></span><span class="lock-icon" aria-hidden="true">⌑</span></button>`).join('')}</div><div class="round-footer">${ready ? '자료 확인 가능' : '발표 자료 준비 중'}</div></article>`;
      }).join('')}</div></section>
...
function renderBody(slide) {
  switch (slide.type) {
    case 'comparison': return `<table class="comparison-table"><thead><tr><th>서비스</th><th>참여 구조</th><th>프로젝트 비교 지점</th></tr></thead><tbody>${slide.rows.map(row => `<tr>${row.map(cell => `<td>${escape(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
    case 'palette': return `<div class="palette-layout"><div class="palette-brand"><span class="eyebrow">BRAND COLOR</span><h2>마젠타</h2><p>#E6007E</p><span>유플러스 대표 컬러</span></div><div class="palette-roles"><article><div class="palette-samples"><i class="sample-ticket"></i></div><div><h2>응모권 · 옐로</h2><p>응모권을 구분하는 포인트 컬러</p></div></article><article><div class="palette-samples"><i class="sample-yellow"></i><i class="sample-pink"></i><i class="sample-lavender"></i></div><div><h2>놀이 · 파스텔</h2><p>옐로 · 핑크 · 라벤더</p></div></article><article><div class="palette-samples"><i class="sample-white"></i><i class="sample-gray"></i></div><div><h2>배경 · 뉴트럴</h2><p>흰색과 그레이 중심 · 과한 색상 사용 줄이기</p></div></article></div></div>`;
    case 'mascot': return `<div class="mascot-layout"><img src="assets/mascot.svg" alt="동그란 추첨공을 띄우는 타코야끼 캐릭터"><div>${slide.reasons.map(reason => `<article><h2>${escape(reason.title)}</h2><p>${escape(reason.text)}</p></article>`).join('')}</div></div>`;
    case 'tech': return `<div class="tech-layout ${slide.pending ? 'is-pending' : ''} ${slide.items.length > 6 ? 'is-dense' : ''}"><div class="tech-list">${slide.items.map(item => `<article class="tech-entry"><img src="assets/tech/${escape(item.icon)}.svg" alt="" width="58" height="58"><div><div class="tech-name"><h2>${escape(item.name)}</h2>${item.version ? `<span class="tech-version">${escape(item.version)}</span>` : ''}</div><p>${escape(item.role)}</p>${item.status ? `<span class="tech-status">${escape(item.status)}</span>` : ''}</div></article>`).join('')}</div><aside class="tech-notes" aria-label="${escape(slide.notesTitle)}"><span class="eyebrow">${escape(slide.notesTitle)}</span>${slide.notes.map(note => `<div><h3>${escape(note.title)}</h3><p>${escape(note.text)}</p></div>`).join('')}</aside></div>`;
    case 'ending': return `<img class="ending-mascot" src="assets/mascot.svg" alt="GETDDO 타코야끼 캐릭터"><div class="ending-copy"><h1>${escape(slide.title)}</h1><p>${escape(slide.description || '')}</p></div>`;
    ...
    case 'agenda': return `<div class="agenda-list ${slide.items.length > 5 ? 'many' : slide.items.length > 3 ? 'compact' : ''}">${slide.items.map((item, i) => `<button class="agenda-item" data-goto="${item.target}"><span>${number(i + 1)}</span><strong>${escape(item.title)}</strong><p>${escape(item.description)}</p><b aria-hidden="true">↗</b></button>`).join('')}</div>`;
    case 'cards': return `<div class="content-card-grid ${slide.cards.length === 4 ? 'four-cards' : slide.cards.length === 2 ? 'two-cards' : ''}">${slide.cards.map(card => `<article class="content-card ${['mint', 'yellow', 'pink'].includes(card.tone) ? card.tone : 'mint'}"><span class="eyebrow">${escape(card.label)}</span><h2>${escape(card.title)}</h2><p>${escape(card.text)}</p><span class="card-tag">${escape(card.tag)}</span></article>`).join('')}</div>`;
...
function renderDeck(round, trackKey, index) {
  ...
  app.innerHTML = `<div class="deck-shell">
    ...
    <div class="slide-footnote">${escape(slide.footnote || '')}</div><footer class="slide-footer">${logo}<span>GETDDO · ${slide.shared ? 'Common' : (tracks[slide.track] || track).english} · MENTORING ${number(round.id)}</span><b>${number(index + 1)} <span>/ ${number(deck.slides.length)}</span></b></footer></article></div></main></div>
...
function renderRoute() {
  ...
  const match = location.hash.match(/^#\/mentoring\/([1-3])\/(frontend|backend|combined)\/(\d+)$/);
